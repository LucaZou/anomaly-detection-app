...
class SimpleNet(torch.nn.Module):
    ...
    def save_segmentation_images(self, data, segmentations, scores):
        ...
        # 按类别和分裂类型（如 test/bent）创建子目录
        for image_path, segmentation, score, mask_path in zip(image_paths, segmentations, scores, mask_paths):
            # 提取类别和子目录信息
            parts = image_path.replace('\\', '/').split('/')  # 统一分隔符
            category = self.dataset_name.replace('mvtec_', '')  # 例如 metal_nut
            split = parts[-3]  # 例如 test
            anomaly_type = parts[-2]  # 例如 bent
            save_folder = os.path.join('./test_result', category, split, anomaly_type)
